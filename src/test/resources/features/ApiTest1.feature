...
Функция: 01. Создание, изменение, получение, удаление пользователя. Администратор системы
  ...
  Сценарий:01. Создание, изменение, получение, удаление пользователя. Администратор системы
    ...
    Если Отправить запрос на изменение пользователя "Пользователь13" пользователем "Пользователь11"
    То Получен статус код ответа 204
    То В базе данных изменилась запись с данными пользователя "Пользователь13"

    Если Отправить запрос на получении инфо о пользователе "Пользователь13" пользователем "Пользователь11"
    То Получен статус код ответа 200
    То В базе данных изменилась запись с данными пользователя "Пользователь13"

    Если Отправить запрос на удаление инфо о пользователе "Пользователь13" пользователем "Пользователь11"
    То Получен статус код ответа 204
    То В базе данных отсутствует информация о  пользователе "Пользователь13" созданном "Пользователь11"

    Если Отправить запрос на удаление инфо о несуществующем пользователе "Пользователь13" пользователем "Пользователь11"
